    ...
    public class Database: IDisposable
    {
        ...
        private void CommonConstruct() {
            ...
            if (_providerName != null) {
                ...
                if (_providerName == "IBM.Data.Informix")
                    _informixprovider = true;
            }
        ...
        public IDbCommand CreateCommand(IDbConnection connection, string sql, params object[] args) {
            ...
            if (_paramPrefix != "@")
                if (_informixprovider)
                    sql = rxParamsPrefix.Replace(sql, m => _paramPrefix);                       // simple string
                else
                    sql = rxParamsPrefix.Replace(sql, m => _paramPrefix + m.Value.Substring(1)); // matchevaluator
                    ...
                    using (var cmd = CreateCommand(_sharedConnection, "")) {
                        ...
                        if (columns == null) {
                            foreach (var i in pd.Columns) {
                                ...
                                if (index > 0)
                                    sb.Append(", ");
                                if (_informixprovider) {
                                    sb.AppendFormat("{0} = {1}", _dbType.EscapeSqlIdentifier(i.Key), _paramPrefix);
                                    ++index;
                                } else {
                            ...
                            foreach (var colname in columns) {
                                ...
                                if (index > 0)
                                    sb.Append(", ");
                                if (_informixprovider) {
                                    sb.AppendFormat("{0} = {1}", _dbType.EscapeSqlIdentifier(colname), _paramPrefix);
                                    ++index;
                                } else {
                        ...
                        if (_informixprovider) {
                            cmd.CommandText = string.Format("UPDATE {0} SET {1} WHERE {2} = {3}",
                                                _dbType.EscapeTableName(tableName), sb.ToString(), _dbType.EscapeSqlIdentifier(primaryKeyName), _paramPrefix);
                            ++index;
                        } else {
